    var myChart = echarts.init(document.querySelector('.chart'),'dark');
    apiready = function () {
        $api.fixStatusBar($api.dom('header'));
        getAllYuanliao().then(option =>  myChart.setOption(option) );
    }
    // 获取所有原料信息，准备做表
    function getAllYuanliao() {
        var promise = new Promise((resolve, reject) => {
            api.ajax({
                ...
                data: {
                    body: {
                        gb_name: 'queryBiz.list#ks02_getCaiLiaoInfo_byCondition',
                        param_info: {}
                    }
                }
            }, function (ret, err) {
                if (ret) {
                    ...
                    var data = ret.param_info.records;
                    var ylArr = [];
                    //日期
                    var arr = [];
                    // 每月数据
                    ...
                    data.forEach((item, index) => {
                        ...
                        for (k in item) {
                            if (k.indexOf('yue') != -1) {
                                data1[index + 1].push(item[k]);
                            }
                        }
                        if(index === 2) {
                            ylArr.push('');
                        }
                        ylArr.push(item.clname);
                    });
