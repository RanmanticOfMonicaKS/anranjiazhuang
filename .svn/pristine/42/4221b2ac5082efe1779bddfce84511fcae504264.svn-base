var appA = "http://122.112.174.170/ksapp/";
...
// 权限认证 游客受限 客户信息部分  手机， 地址，编号，全部模糊 
function makeDim() {
    var userInfo = $api.getStorage('userInfo') || {};
    if(!userInfo.userstatus) {
        $('.kh_phone').addClass('text-mask');
        $('.kh_address').addClass('text-mask');
    }
}
