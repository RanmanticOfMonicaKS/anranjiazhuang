...
function checkLogin() {
    ...
    console.log(JSON.stringify(userName) + '---' + JSON.stringify(userPwd));

    if (!userName) return alert('账号不能为空');
    if (!userPwd) return alert('密码不能为空');
    api.showProgress({
        title: '正在登录',
        text: ' 请稍后',
        modal: true
    ...
    api.ajax({
        ...
        data: {
            body: {
                gb_name: 'queryBiz.page#ks02_getUserInfo_byCondition',
                userPwd,
                userName
            }
        }
        ...
        if (ret) {
            // TODO 
            api.hideProgress();
            console.log(JSON.stringify(ret) + '---' + JSON.stringify(userName) + '---' + JSON.stringify(
                userPwd));
            var IDs = ret.param_info.records;
            var person = [];
            person = IDs.filter(item => item.username == userName && item.userpwd == userPwd);
            console.log(JSON.stringify(person));

            if (person.length) {

                console.log(JSON.stringify(person));
                var userInfo = person[0];
                $api.setStorage('userInfo', userInfo);
                // 再次登录时，的验证
                // api.execScript({
                //     frameName: 'main_table',
                //     script:'checkMan();'
                // });
                api.sendEvent({
                    name: 'sameTheNic',
...
function makeTheSame() {
    index = $api.getStorage('frameGroupIndex');

}

function closeOldFrames() {
    api.closeFrameGroup({
        name: 'mainGroup'
    });
}
