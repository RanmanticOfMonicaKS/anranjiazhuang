    ...
    function getFromData() {
        ...
        ddinfo.fgName = fgName;
        console.log(JSON.stringify(ddinfo) + '----' + JSON.stringify(fgName));
         ...
         */
        if (!ddinfo.useTime || !Number(ddinfo.useTime)) {
            return api.toast({
                msg: '请输入正确的工期格式',
                duration: 2000,
                location: 'top'
        ...
        //验证khId唯一性 最后验证
        checkKhId(ddinfo.ddId, ddinfo);

    }

    // 检查khid是否唯一的函数
    function checkKhId(ddId, ddinfo) {
        console.log(JSON.stringify(ddId) + '----' + JSON.stringify(ddinfo));
